...
#             # Chart 2: Word Cloud (if desired)
#             # Uncomment the following lines if you want to add a word cloud
#             # wordcloud = generate_word_cloud(filtered_comments)
#             # if wordcloud:
#             #     st.subheader(f"Word Cloud for {selected_sentiment.capitalize()} Comments")
#             #     plt.figure(figsize=(10, 5))
...
#          # Additional: Display DataFrame of comments
#         st.subheader(f"All {selected_sentiment.capitalize()} Comments")
...
if st.sidebar.checkbox("Sentimental Analysis"):
    ...
    if st.sidebar.button("Analyze Sentiments"):
        ...
        fig_polarity = px.bar(x=list(categorized_comments.keys()), y=[len(categorized_comments[key]) for key in categorized_comments],
                              labels={'x': 'Sentiment', 'y': 'Count'},
                              title="Sentiment Distribution of Comments")
        fig_polarity.update_layout(height=600, width=1000)  # Increased size for better visibility
        st.plotly_chart(fig_polarity)
