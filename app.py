...
# Cache channel data to prevent reloading on filter changes
@st.cache_data(ttl=3600, show_spinner="Fetching channel data...")
def get_channel_analytics(channel_id):
    try:
        # Get channel statistics
...
def youtube_dashboard():
    ...
    if analyze_btn and channel_id:
        ...
        video_df["published_at"] = pd.to_datetime(video_df["published_at"])
        video_df["duration_sec"] = video_df["duration"].apply(lambda x: int(x[2:-1]) if 'M' in x else 0)
        video_df["duration_formatted"] = video_df["duration_sec"].apply(lambda x: f"{x//60}:{x%60:02d}")
        video_df["engagement"] = video_df["engagement"].round(2)
        video_df["publish_day"] = video_df["published_at"].dt.day_name()
        video_df["publish_month"] = video_df["published_at"].dt.strftime("%Y-%m")
        video_df["publish_hour"] = video_df["published_at"].dt.hour
        ...
        with col_header1:
            st.image(channel_data["basic_info"]["thumbnail"], width=120)
            
        with col_header2:
            st.markdown(f"### {channel_data['basic_info']['title']}")
            st.markdown(f"**Subscribers:** {format_number(channel_data['statistics']['subscriber_count'])} | **Total Views:** {format_number(channel_data['statistics']['view_count'])} | **Videos:** {channel_data['statistics']['video_count']}")
            st.markdown(f"**Created:** {datetime.strptime(channel_data['basic_info']['published_at'], '%Y-%m-%dT%H:%M:%SZ').strftime('%B %d, %Y')}")
        
        st.markdown("---")
        
        # Top Insights in 5-column grid
        ...
        with st.container():
            ...
            if top_video is not None:
                st.markdown(f"""
                <div class="insight-card">
                    <div class="insight-title">🎯 Top Video</div>
                    <div class="insight-value">{format_number(top_video['views'])} views</div>
                    <div class="insight-description">
                        "{top_video['title'][:30]}{'...' if len(top_video['title']) > 30 else ''}"
                    </div>
                </div>
                """, unsafe_allow_html=True)
            ...
            <div class="insight-card">
                ...
                <div class="insight-value">{avg_engagement:.1f}%</div>
                <div class="insight-description">
            ...
            # Insight 3: Optimal Length
            st.markdown(f"""
            <div class="insight-card">
                <div class="insight-title">⏱️ Best Length</div>
                <div class="insight-value">{optimal_bin}</div>
                <div class="insight-description">
            ...
            st.markdown(f"""
            <div class="insight-card">
                <div class="insight-title">⏰ Best Hour</div>
                <div class="insight-value">{best_hour_str}</div>
                <div class="insight-description">
                    To publish videos
                </div>
            ...
            # Insight 6: Like Ratio
            st.markdown(f"""
            <div class="insight-card">
                <div class="insight-title">👍 Like Ratio</div>
                <div class="insight-value">{avg_like_ratio:.1f}%</div>
                <div class="insight-description">
                    Likes per view
                </div>
            </div>
            ...
            # Insight 7: Comment Ratio
            st.markdown(f"""
            <div class="insight-card">
                <div class="insight-title">💬 Comment Ratio</div>
                <div class="insight-value">{avg_comment_ratio:.1f}%</div>
                <div class="insight-description">
                    Comments per view
                </div>
            </div>
            ...
            # Insight 8: Publishing Frequency
            st.markdown(f"""
            <div class="insight-card">
                <div class="insight-title">⏳ Upload Frequency</div>
                <div class="insight-value">{avg_days_between:.1f} days</div>
                <div class="insight-description">
                    Between videos
                </div>
            ...
            # Insight 10: Avg Views
            st.markdown(f"""
            <div class="insight-card">
                <div class="insight-title">📊 Avg Views</div>
                <div class="insight-value">{format_number(avg_views)}</div>
                <div class="insight-description">
                    Per video
                </div>
            </div>
        ...
        # Performance Analytics with session state to maintain filters
        if 'chart_data' not in st.session_state:
            st.session_state.chart_data = video_df.copy()
        
        st.subheader("📈 Performance Analytics")
        
        tab1, tab2 = st.tabs(["Views Analysis", "Engagement Metrics"])
        ...
        with tab1:
            ...
            if st.session_state.get('prev_views_filters') != (min_views, duration_filter):
                ...
                st.session_state.chart_data = filtered_df
                st.session_state.prev_views_filters = (min_views, duration_filter)
        ...
        with tab2:
            ...
            # Apply filters without reloading
            if st.session_state.get('prev_eng_filters') != (min_engagement, view_filter):
                ...
                st.session_state.chart_data = filtered_df
                st.session_state.prev_eng_filters = (min_engagement, view_filter)
