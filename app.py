...
def get_date_window(window):
    if isinstance(window, list) and len(window) == 2:
        return pd.to_datetime(window[0]), pd.to_datetime(window)
...
def get_videos(client, channel_id, max_results=50):
    ...
    while len(v_ids) < max_results:
        req = client.search().list(
            ...
            maxResults=min(50, max_results - len(v_ids)),
            type="video",
            order="date",
...
if channel_id:
    ...
    if "Top 5 Performing Videos (Views)" in selected:
        ...
        df5 = pd.DataFrame({"Title": v_titles, "Views": views}).sort_values("Views", ascending=False).head(5)
    ...
    if "Audience Geography" in selected:
        ...
        fig = px.choropleth(geo_df, locations="Country", locationmode="country names", color="Views",
                            color_continuous_scale=["#ff0000"], title="Audience Geography")
        st.plotly_chart(fig, use_container_width=True)

    if "Gender Distribution" in selected:
        g_df = pd.DataFrame({"Gender": ["Male", "Female", "Other"], "Percent": [62, 34, 4]})
        fig = px.pie(g_df, names="Gender", values="Percent", hole=0.6,
                     color_discrete_sequence=["#ff0000", "#d63b3b", "#bf5555"], title="Gender Distribution")
        st.plotly_chart(plotly_settings(fig), use_container_width=True)

    if "Age Distribution" in selected:
        a_df = pd.DataFrame({"Age Group": ["13-17", "18-24", "25-34", "35-44", "45+"], "Percent": [3, 28, 34, 26, 9]})
        fig = px.bar(a_df, x="Percent", y="Age Group", orientation="h",
                     color_discrete_sequence=["#ff0000"], title="Age Distribution")
        st.plotly_chart(plotly_settings(fig), use_container_width=True)
    ...
    if "Engagement Rate (Likes, Comments, Shares)" in selected:
        ...
        fig = px.bar(df, x="Title", y=["Likes", "Comments", "Shares"], barmode="stack", title="Engagement Rate",
                     color_discrete_sequence=["#ff0000", "#d63b3b", "#bf5555"])
        st.plotly_chart(fig, use_container_width=True)

    if "Viewer Retention Over Time" in selected:
        mins = np.arange(1, 11)
        retain = np.maximum(100 - np.cumsum(np.random.poisson(7, 10)), 0)
        fig = px.area(x=mins, y=retain, labels={'x': 'Minutes', 'y': 'Retention (%)'}, title="Viewer Retention Over Time",
    ...
    if "Device Distribution (Mobile vs Desktop)" in selected:
        ...
        fig = px.pie(names=devices, values=device_val, hole=0.5, title="Device Distribution",
    ...
    if "Playlist Performance" in selected:
        ...
        fig = px.line_polar(pl_df, r="Views", theta="Playlist", line_close=True, title="Playlist Performance",
                            color_discrete_sequence=["#ff0000"])
        st.plotly_chart(fig, use_container_width=True)
    ...
    if "SEO Score per Video (tags/titles)" in selected:
        ...
        fig = px.scatter(seo_df, x="Video", y="SEO Score", size="SEO Score", title="SEO Score per Video",
                         color_discrete_sequence=["#ff0000"])
        st.plotly_chart(fig, use_container_width=True)
    ...
    if "Subscriber Gain/Loss Events" in selected:
        ...
        fig = px.scatter(x=dates, y=gain - loss, title="Subscriber Net Gain/Loss", labels={'y': 'Net Change'},
                         color_discrete_sequence=["#ff0000"])
        st.plotly_chart(fig, use_container_width=True)
    ...
    if "Revenue Estimates (if monetized)" in selected:
        ...
        fig = px.line(x=months, y=revenue, title="Estimated Revenue", labels={'x': 'Month', 'y': 'USD'},
                      color_discrete_sequence=["#ff0000"])
        st.plotly_chart(fig, use_container_width=True)
